...
\chapter{Parte B}
	...
	\section{2-COLOR es polinomial}
		\underline{Teorema:} "-Color es polinomial, es decir, existe un algoritmo polinomial que lo resuelve.

		\underline{Prueba:}
			\par Consideremos el siguiente algoritmo con entrada $G = (V, E)$

			\vspace{5mm}
			\par \textbf{Algoritmo:}
			\par For each COMPONENTE CONEXA:
				\begin{enumerate}
					\item Elegir x
					\item Correr BFS(x)
					\item Colorear vertices con: $\qquad	C(v) = Nivel_{BFS(x)}(v) \; mod \; 2$
					\item Chequear que el coloreo de (3) sea propio. Si lo es, RETURN "SI", si no lo es, RETURN "NO".
				\end{enumerate}

			Veamos ahora que el algoritmo es correcto y que su complejidad es polinomial.
			\begin{itemize}
				\item Complejidad Polinomial:
					\begin{itemize}
						\item (1) es $\mathcal{O}(1)$.
						\item (2) es $\mathcal{O}(m + c)$ (recorrer todos los vértices). Donde \textit{c} es una costante que significa \# vértices aislados.
						\item (3) es $\mathcal{O}(m)$ (recorrer todos los vértices).
				...
				\item Correctitud:
					\begin{itemize}
						\item Cuando dice "SI": Obviamente dice "SI", solo si el coloreo es propio y si el coloreo es propio, $\chi(G) \leq 2$.
						\item Cuando dice "NO": Tenemos que ver que cuando dice "NO" ningún coloreo propio con 2 colores existe.
							\par Por otro teorema visto en clase, si existe un ciclo impar $C_{2k+1} \subseteq G \Rightarrow \chi(G) \geq 3$. Por lo que bastaría ver que si el algoritmo dice "NO", entonces exista un cliclo impar en G.
							\par Si el algoritmo dice "NO" $\Rightarrow \exists z, y : z y \in E(G) \textup{ y } C(z) = C(y) \Rightarrow Nivel_{BFS(x)}(z) = Nivel_{BFS(x)}(y)$
							\par Sea:
							\begin{itemize}
